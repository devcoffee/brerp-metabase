--Fluxo de caixa : agrupada saldos não conciliados, ajusta pagamentos em contas transitórias de crédito (eX CARTÃO), ajusta cheques a compensasr e planos de caix
WITH fluxo as (
...
WHERE
	temp.ad_client_id = 1000000
AND
      (case WHEN {{TipoP}}='01' then 
                        temp.ad_org_id IN (1000001) 
           WHEN {{TipoP}}='02' then 
                    temp.ad_org_id IN (5000000) 
           WHEN {{TipoP}}='03' then 
                        temp.ad_org_id IN (5000004) 
           WHEN {{TipoP}}='98' then 
                        temp.ad_org_id IN (5000000,1000001)
           else 
                        temp.ad_org_id IN (5000004,5000000,1000001)
           end )
	--PROJEÇÃO PASSADA
AND 
    CASE WHEN 'N' = 'N' THEN
